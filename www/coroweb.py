import asyncio
import os
import inspect
import logging
import functools

from urllib import parse
from aiohttp import web
from apis import APIError


logging.basicConfig(level=logging.INFO)


def get(path):
    """
    Define decorator @get('/path')
    """
    def decorator(f):
        @functools.wraps(f)
        def wrapper(*args, **kw):
            return f(*args, **kw)
        wrapper.__method__ = "GET"
        wrapper.__route__ = path
        return wrapper
    return decorator


def post(path):
    """
    Define decorator @post("/path")
    """
    def decorator(f):
        @functools.wraps(f)
        def wrapper(*args, **kw):
            return f(*args, **kw)
        wrapper.__method__ = "POST"
        wrapper.__route__ = path
        return wrapper
    return decorator


def get_required_kw_args(f):
    args = []
    params = inspect.signature(f).parameters
    for name, param in params.items():
        if param.kind == inspect.Parameter.KEYWORD_ONLY and \
                param.default == inspect.Parameter.empty:
            args.append(name)
    return tuple(args)


def get_named_kw_args(f):
    args = []
    params = inspect.signature(f).parameters
    for name, param in params.items():
        if param.kind == inspect.Parameter.KEYWORD_ONLY:
            args.append(name)
    return tuple(args)


def has_named_kw_args(f):
    return get_named_kw_args(f) != ()


def has_var_kw_args(f):
    params = inspect.signature(f).parameters
    for name, param in params.items():
        if param.kind == inspect.Parameter.VAR_KEYWORD:
            return True


def has_request_args(f):
    sig = inspect.signature(f)
    params = sig.parameters
    ret = False
    for name, param in params.items():
        if name == "request":
            ret = True
            continue
        if ret and param.kind != inspect.Parameter.VAR_POSITIONAL \
                and param.kind != inspect.Parameter.KEYWORD_ONLY \
                and param.kind != inspect.Parameter.VAR_KEYWORD:
            raise ValueError("request parameter must be the last named parameter in function: %s%s"
                             % (f.__name__, str(sig)))
    return ret


class RequestHandler(object):
    def __init__(self, app, f):
        self._app = app
        self._func = f
        self._has_request_args = has_request_args(f)
        self._has_var_kw_args = has_var_kw_args(f)
        self._has_named_kw_args = has_named_kw_args(f)
        self._named_kw_args = get_named_kw_args(f)
        self._required_kw_args = get_required_kw_args(f)

    async def __call__(self, request):
        kw = None
        if self._has_var_kw_args or self._has_named_kw_args or self._required_kw_args:
            if request.method == "POST":
                if not request.content_type:
                    return web.HTTPBadRequest("Missing Content-Type.")
                content_type = request.content_type
                if content_type.startswith("application/json"):
                    params = await request.json()
                    if not isinstance(params, dict):
                        return web.HTTPBadRequest("JSON body must be object.")
                    kw = params
                elif content_type.startswith("application/x-www-form-urlencoded") \
                        or content_type.startswith("multipart/form-data"):
                    params = await request.post()
                    kw = dict(**params)
                else:
                    return web.HTTPBadRequest("Unsupported Content-Type: %s" % request.content_type)
            if request.method == "GET":
                query = request.query_string
                if query:
                    kw = dict()
                    for k, v in parse.parse_qs(query, True).items():
                        kw[k] = v[0]

        if kw is None:
            kw = dict(**request.match_info)
        else:
            if not self._has_var_kw_args and self._named_kw_args:
                tmp = dict()
                for name in self._named_kw_args:
                    if name in kw:
                        tmp[name] = kw[name]
                kw = tmp

            for k, v in request.match_info.items():
                if k in kw:
                    logging.warning("Duplicated arg name in named arg and kw args: %s" % k)
                kw[k] = v

        if self._has_request_args:
            kw["request"] = request
        if self._required_kw_args:
            for name in self._required_kw_args:
                if name not in kw:
                    return web.HTTPBadRequest("Missing argument: {}".format(name))
        logging.info("call with args: %s" % str(kw))

        try:
            ret = await self._func(**kw)
            return ret
        except APIError as e:
            return dict(error=e.error, data=e.data, message=e.message)


def add_static(app):
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static")
    app.router.add_static("/static/", path)
    logging.info("add static %s => %s" % ("/static/", path))


def add_route(app, f):
    method = getattr(f, "__method__", None)
    path = getattr(f, "__route__", None)
    if path is None or method is None:
        raise ValueError("@get or @post not defined in %s." % str(f))
    if not asyncio.iscoroutinefunction(f) and not inspect.isgenerator(f):
        f = asyncio.coroutine(f)
    logging.info("add route %s %s => %s(%s)" %
                 (method, path, f.__name__, ", ".join(inspect.signature(f).parameters.keys())))
    app.router.add_route(method, path, RequestHandler(app, f))


def add_routes(app, module_name):
    n = module_name.rfind(".")
    if n == -1:
        mod = __import__(module_name, globals(), locals())
    else:
        name = module_name[n + 1:]
        mod = getattr(__import__(module_name[:n], globals(), locals(), [name]), name)

    for attr in dir(mod):
        if attr.startswith("_"):
            continue
        f = getattr(mod, attr)
        if callable(f):
            method = getattr(f, "__method__", None)
            path = getattr(f, "__route__", None)
            if method and path:
                add_route(app, f)
